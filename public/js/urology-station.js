...
<div class="stitch-station-container p-6 bg-slate-50 min-h-screen">
  <header class="flex justify-between items-center mb-6">
    ...
    <div class="flex gap-2">
      <button onclick="UrologyStation.openStoneLog()" class="stitch-btn-primary px-4 py-2 bg-yellow-600 text-white rounded-lg shadow-sm hover:bg-yellow-700">${tr('Log Stone', 'تسجيل حصوة')}</button>
      <button onclick="UrologyStation.openPSA()" class="stitch-btn-secondary px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg shadow-sm hover:bg-slate-100">${tr('PSA Trend', 'اتجاه PSA')}</button>
    </div>
  </header>
  <div class="grid grid-cols-12 gap-6">
    ...
    <div class="col-span-6 space-y-6">
      <div class="stitch-card p-6 bg-white rounded-xl shadow-sm border border-slate-200 min-h-[500px]">
        <div class="flex border-b border-slate-200 mb-6">
          <button class="px-4 py-2 border-b-2 border-yellow-600 text-yellow-600 font-medium">${tr('Stone Log', 'سجل الحصوات')}</button>
          <button class="px-4 py-2 text-slate-500 hover:text-slate-800">${tr('PSA Trend', 'اتجاه PSA')}</button>
          <button class="px-4 py-2 text-slate-500 hover:text-slate-800">${tr('Urodynamics', 'ديناميكا البول')}</button>
        </div>
    ...
    openStoneLog: () => alert('Stone log (POST /api/urology/stone-log)'),
    openPSA: () => alert('PSA log (POST /api/urology/oncology-metrics)')
};
